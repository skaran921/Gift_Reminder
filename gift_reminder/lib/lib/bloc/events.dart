  ...
  Future applyAsyncEvent({GiftAppState currentState, GiftAppBloc bloc}) async {
    try {
      ...
      await TransactionService.getTransaction(adminId: AdminToken.adminId)
        ...
        await BookService.getBooks(adminId: AdminToken.adminId).then((value) {
          bloc.books = value['results'] ?? [];
          bloc.books.insert(0, {
            "BOOK_ID": "",
            "BOOK_NUMBER": "Select Book",
            "ADMIN_ID": AdminToken.adminId,
            "BOOK_CREATE_DATE": "2020-05-3104:15:13",
            "BOOK_UPDATE_DATE": "2020-05-3104:15:13",
            "BOOK_STATUS": "ACTIVE"
          });
        });
      });
...
//  AddBookData Evvent
class AddBookDataEvent extends GiftAppEvent {
  final List data;
  AddBookDataEvent(this.data);
  @override
  Future applyAsyncEvent({GiftAppState currentState, GiftAppBloc bloc}) async {
    try {
      bloc.books.add(data[0]);
      bloc.isAddBookLoading = false;
      return InGiftAppDashBoardState();
    } catch (error) {
      return ErrorGiftAppState(error);
    }
...
//  SetBookLoading Evvent
class SetBookLoadingEvent extends GiftAppEvent {
  final bool isLoading;
  SetBookLoadingEvent(this.isLoading);
  @override
  Future applyAsyncEvent({GiftAppState currentState, GiftAppBloc bloc}) async {
    try {
      bloc.isAddBookLoading = isLoading;
      return InGiftAppDashBoardState();
    } catch (error) {
      return ErrorGiftAppState(error);
    }

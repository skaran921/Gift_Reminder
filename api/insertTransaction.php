...
if(
    ...
    if($result->num_rows){
             $insert_sql = "INSERT INTO transaction(NAME,ADDRESS,AMOUNT,PAGE_NUMBER,BOOK_ID,ADMIN_ID)values('$name','$address','$amount','$page_no','$book_id','$admin_id')";

             $insert_result = $conn->query($insert_sql);
             if($insert_result){
                //    * if transaction save
                $msg = array("msg"=>"Transaction Added","error"=>"","result"=>array("id"=>$conn->insert_id));
